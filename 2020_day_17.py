...
def get_neighbours_3_dim(cubeset, x, y, z):
    active_neighbours = {}
    inactive_neighbours = {}
...
def get_neighbours_4_dim(cubeset, x, y, z, w):
    active_neighbours = {}
    inactive_neighbours = {}
    for i in range(x - 1, x + 2):
        for j in range(y - 1, y + 2):
...
if __name__ == '__main__':
    ...
    for cycle in range(0, 6):
        ...
        temp_cubes_4 = {}
        for cube in cubes_3:
            neighbours = get_neighbours_3_dim(cubes_3, cube[0], cube[1], cube[2])
            if len(neighbours[1]) in [2, 3]:
                temp_cubes_3[cube] = 1
            for inac_neigh in neighbours[0]:
                if len(get_neighbours_3_dim(cubes_3, inac_neigh[0], inac_neigh[1], inac_neigh[2])[1]) == 3:
                    temp_cubes_3[inac_neigh] = 1
        cubes_3 = temp_cubes_3
        for cube in cubes_4:
            neighbours = get_neighbours_4_dim(cubes_4, cube[0], cube[1], cube[2], cube[3])
            if len(neighbours[1]) in [2, 3]:
                temp_cubes_4[cube] = 1
            for inac_neigh in neighbours[0]:
                if len(get_neighbours_4_dim(cubes_4, inac_neigh[0], inac_neigh[1], inac_neigh[2], inac_neigh[3])[1]) == 3:
